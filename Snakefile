...
rule link_datasets:
    ...
    shell:
        f'{config["conda_path"]} run --no-capture-output -n {config["conda_environment_name"]} papermill {{input[0]}} {{log}} {dict_to_papermill(link_datasets_papermill_params)} -k python3'
